...
void read_pipe() {
    ...
    if (pipe_fd == -1) {
        ...
        perror("Failed to open named pipe for read -- Person 1");
...
void write_pipe() {
    ...
    if (pipe_fd == -1) {
    	...
    	perror("Failed to open named pipe for writing -- Person 1");
		
       	exit(EXIT_FAILURE);
		
    }

    printf("Enter text (type 'GoodBye' to quit):\n");
